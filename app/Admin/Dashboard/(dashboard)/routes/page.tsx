...
export default function CarFareManager() {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const res = await fetch("https://dawood-backend.vercel.app//cars/all-cars");
  ...
  const fetchFares = async (carId: string) => {
    setLoading(true);
    try {
      const res = await fetch(`https://dawood-backend.vercel.app//fare/${carId}`);
  ...
  const handleFareUpdate = async (fareId: string, newFare: number) => {
    try {
      await fetch(`https://dawood-backend.vercel.app//fare/${fareId}`, {
        method: "PUT",
  ...
  const handleAddFare = async () => {
    ...
    try {
      await fetch("https://dawood-backend.vercel.app//fare/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          car: selectedCarId,
